...
실무에서 세그먼트 도구를 만들다 보면 마주치는 방식: 카테고리(도메인)별로 원본 테이블을 미리
반정규화해 고객 키가 박힌 넓은 마트로 만들어두는 접근. 조인은 쉬워지지만 카테고리 하나 늘
때마다 마트를 새로 설계·구축해야 함 (ETL, 중복 저장, 최신성 저하).
...
| 초기 설계 | 변경 |
|---|---|
| 테이블을 사용자가 직접 등록 (STATE/HISTORY 수동 입력) | `information_schema` + 네이밍 컨벤션으로 자동 판정 — 이미 DB 스키마에 다 있는 정보였음 |
| 드래그앤드롭으로 중간 테이블 직접 연결, 그래프 탐색은 배제 | BFS 최단경로 기반 자동 연결 — 조건 먼저 고르는 흐름에선 몇 테이블 거쳐야 하는지 사용자가 알 수 없었음 |
| 조인 화면과 조건 화면을 분리 | 조건 화면 하단 실시간 패널로 통합 — 자동연결 후 조인 화면에서 할 일이 거의 없어져 페이지만 왕복하던 구조였음 |

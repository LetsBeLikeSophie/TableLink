...
- **RLS 구축 중 겪은 두 가지**: Postgres RLS 정책 생성을 `DO $$ ... $$` 블록 하나로 묶었더니
  `psql`로는 되는데 Spring Boot가 `schema.sql`을 실행할 때는 "unterminated dollar quote"로
  깨졌습니다 — Spring의 스크립트 실행기는 `;` 기준으로만 문장을 나눠서 Postgres 전용
  달러 인용 구문을 모릅니다. 테이블 7개분을 반복문 없이 풀어 썼습니다. 그리고
  `SELECT set_config(...)`를 `JdbcTemplate.update()`로 실행했다가 "결과가 반환됐는데
  없어야 한다"는 에러가 났습니다 — `set_config`는 값을 반환하는 **쿼리**라 `queryForObject`로
  바꿔야 했습니다.
...
"로그인한 유저마다 자기 국가의 데이터만 봐야 한다"는 요구를 가정하고 실제로 붙여본
기능입니다. 라이브 데모에 로그인 화면이 있고, 계정 두 개(`kr_user`/`us_user`, 비밀번호
`tablelink1234`)로 각각 한국어/영어 데이터셋만 보이는 걸 확인할 수 있습니다.
...
1. GUC 미설정 시 결과가 조용히 0건 — 에러가 아니라서 디버깅 함정
2. 원래 오토커밋이던 조회를 트랜잭션으로 감싸는 오버헤드
3. 세션 쿠키가 이중 프록시(엣지→오리진)를 거치므로 `credentials: 'include'`, 쿠키 도메인/경로 확인 필요
4. "쉬운 비밀번호 + 해시"는 유출 방지일 뿐, rate limiting 없인 무차별 대입에 그대로 노출
5. `country`가 인증 스코프·언어·데이터 테넌시를 한 필드로 겸함 — 실제 시스템이면 분리가 맞음
6. 유저 2명짜리 문제에 DB 레벨 RLS까지 쓰는 건 문제 크기 대비 과함 — 여기선 기법을 보여주려는 의도적 선택

...
1. **조건 선택 & 조인 (한 화면)**
   ...
   - 클릭해서 담으면 operator(컬럼 valueType에 따라 후보 제한: CATEGORY→EQ/NEQ,
     ...
     있어야 하는 게 아니라 실제 데이터의 카디널리티로 즉석 판단)
...
2. **세그먼트 결과** — `POST /segments` 호출 → 매칭된 루트 테이블(대부분 customer) row 목록 +
...
갱신), 2단계는 결과 화면 완성 후 표시 예정. 콘텐츠 길이가 스텝마다 달라서 인라인에 두면 위치가
들쭉날쭉해지는 문제가 있어 앱 레벨 고정 레이아웃으로 뺐다.
